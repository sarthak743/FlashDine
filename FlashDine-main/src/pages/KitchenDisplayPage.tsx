...
import { RestaurantAuthModal } from '@/components/RestaurantAuthModal';
...
const normalizeRestaurantId = (restaurantId?: string | null) => {
  if (!restaurantId) return DEFAULT_RESTAURANT_ID;
...
export function KitchenDisplayPage() {
  const { orders, updateOrderStatus, updateOrderEstimatedTime, banReceipt, addOrder, restaurantId, setRestaurantId } = useStore();
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const callbackRestaurantId = new URLSearchParams(hashQuery).get('restaurantId');
      const cachedRestaurantId = getRestaurantId() || callbackRestaurantId;
  ...
  const handleAuthSuccess = (authRestaurantId: string) => {
  ...
  const activeRestaurantId = normalizeRestaurantId(restaurantId);
  const restaurantScopedOrders = orders.filter(
    (order) => normalizeRestaurantId(order.restaurantId) === activeRestaurantId
  );

  const filteredOrders = restaurantScopedOrders.filter((order) => {
    if (filterStatus === 'all') return true;
    return order.status === filterStatus;
  }).sort((left, right) => (left.token ?? Number.MAX_SAFE_INTEGER) - (right.token ?? Number.MAX_SAFE_INTEGER));
  ...
  return (
    <>
      {/* Auth Modal */}
      <RestaurantAuthModal isOpen={showAuthModal} onAuthSuccess={handleAuthSuccess} />
        ...
        {restaurantId && (
          <div className="bg-zinc-900 border-b border-zinc-800 px-4 py-3 flex justify-between items-center">
            <p className="text-zinc-400 text-sm">
              <span className="font-semibold text-white">Restaurant ID:</span> {restaurantId === DEFAULT_RESTAURANT_ID ? CAMPUS_DELIGHTS_ALIAS : restaurantId}
            </p>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded-lg transition-all text-sm border border-red-500/30"
            >
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {(['all', 'received', 'preparing', 'ready', 'completed'] as FilterStatus[]).map((status) => {
              const count = status === 'all' 
                ? restaurantScopedOrders.length 
                : restaurantScopedOrders.filter((o) => o.status === status).length;
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3 bg-orange-500/10 border border-orange-500/20 rounded-lg px-3 py-2">
                      <span className="text-orange-300 text-sm font-semibold">Token</span>
                      <span className="text-orange-400 font-black text-lg">#{order.token}</span>
                    </div>
