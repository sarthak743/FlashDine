...
export function MenuPage() {
  ...
  useEffect(() => {
    ...
    // Fallback initialize stock with menu items
    initializeStock(menuItems);

    // Fetch from backend to override with true stock statuses
    const targetRestaurantId = restaurantId || (qrData ? getRestaurantByQRCode(qrData)?.id : 'default');
    if (targetRestaurantId) {
      getMenuStock(targetRestaurantId).then((stockData) => {
        // Only load if we have data from backend, otherwise let the fallback persist
        if (Object.keys(stockData).length > 0) {
           // We'll merge defaults missing from backend with the backend state
           const mergedStock = { ...useStore.getState().menuStock };
           Object.assign(mergedStock, stockData);
           initializeStock(mergedStock);
        }
  ...
  const recentItems = recentlyOrdered
    ...
    .filter((item): item is import('@/types').MenuItem => Boolean(item))
    .slice(0, 4);
