...
export function AdminDashboardPage() {
  ...
  const { restaurantId, setRestaurantId, setIsAdmin } = useStore();
  ...
  // Verify active session on mount/refresh.
  useEffect(() => {
    const checkAuth = async () => {
      const cachedRestaurantId = getRestaurantId();
      ...
      try {
        ...
        setRestaurantId(verifiedRestaurantId);
        setIsAdmin(true);
      } catch {
  ...
  if (!restaurantId) {
    return (
      <div className="min-h-screen bg-zinc-900 flex items-center justify-center">
        <p className="text-zinc-400">Loading restaurant dashboard...</p>
      </div>
    );
  }
  ...
  const sortedOrders = [...orders].sort(
    (left, right) => (left.token ?? Number.MAX_SAFE_INTEGER) - (right.token ?? Number.MAX_SAFE_INTEGER)
  );
                  ...
                  <div className="p-4 border-b border-zinc-700/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="text-right">
                        <p className="text-zinc-400 text-xs uppercase tracking-wider">Token</p>
                        <p className="text-white font-black text-lg">#{order.token}</p>
